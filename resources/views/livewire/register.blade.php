<div>
    ...
    <div class="bg-gradient-to-tr from-green-300 to-green-600 h-screen w-full flex justify-center items-center">
        ...
        <div
            class="bg-green-600 w-full max-w-500 shadow-md flex flex-col md:flex-row items-center mx-5 sm:m-0 rounded my-5">

            <div class="bg-white w-full  flex flex-col items-center py-10 px-8  justify-center">
                <h3 class="text-3xl font-bold text-green-500 mb-4">Đăng ký</h3>

                <form wire:submit.prevent='submit' autocomplete="none" class="w-full flex flex-col justify-center">
                    <div class="mb-4">
                        <label for="">Họ tên</label>

                        @error('user.name')
                                <span class=" text-sm text-red-500 ml-3">{{$message}}</span>
                        @enderror
                        <input wire:model='user.name' type="text" placeholder="Họ tên"
                    ...
                    <div class="mb-4">
                        ...
                        @error('password')
                                <span class=" text-sm text-red-500 ml-3">{{$message}}</span>
                        @enderror
                        <input wire:model='password' type="password" placeholder="Password" required="" name="password"
                    ...
                    <div class="mb-4">
                        ...
                        @error('password_confirmation')
                                <span class=" text-sm text-red-500 mr-3">{{$message}}</span>
                        @enderror
                        <input wire:model='password_confirmation' type="password" placeholder="Xác nhận mật khẩu" required="" name="password"
                            autocomplete="current-password"
                            class="w-full p-3 rounded border placeholder-gray-400 focus:outline-none focus:border-green-600">
                    </div>
                    ...
                    ký</button>
                        {{-- <div class="flex justify-between mb-3"> --}}
                            <a class="text-sm font-semibold text-blue-500" href="{{url('dang-nhap')}}">Quên
                                Đã có tài khoản?</a>
